...
def collectData(data, mae4, colt, start_time, disturbance, reward, trial = False):
...
def stage_one(kit, trial_led, led, mae4, colt, colt_fun, start_time):
    ...
    reward_distance = 1
    lever_displacement = 5 #to trigger reward block
    #==============================
    ...
    def reward_loop(led):
        ...
        while not stop_threads.is_set():
            if reward_active:
                for _ in range(reward_block_duration):
                    if stop_threads.is_set():
                        break
                    dispense_reward(led)
                    time.sleep(reward_distance)
                reward_active = False

    def data_loop(kit, trial_led, led, mae4, colt, colt_fun):
        global reward_active
        while not stop_threads.is_set():
            ...
            collectData(data, lever_angle, servo_angle, time.time(), 0, 0, False)

            if lever_angle >= lever_displacement:
                reward_active = True

    try:
        updateServoPos(kit, ADJUSTED_CENTER)

        data_thread = threading.Thread(target=data_loop, args=(kit, trial_led, led, mae4, colt, colt_fun), daemon=True)
        reward_thread = threading.Thread(target=reward_loop, args=(led,), daemon=True)
...
def main():
    ...
    reward_timer = 0
    trial_timer = 0
    trial_distance_timer = 0
    in_trial = True
    trial_distance = random.randint(TRIAL_DISTANCE_RANGE)
    trial_count = 1
    ...
    while True:
        if ENABLE_TRIALS:
            ...
            servo_angle = (colt_v - b) / m
            if in_trial:
                if time.time() - trial_timer >= TRIAL_LENGTH:
                    trial_timer = time.time()
                    in_trial = False
                    trial_led.off()
                    updateServoPos(kit, 3, 0)
                    collectData(data, lever_angle, servo_angle, start_time, 0, 0, 0)
                else:
                ...
                if time.time() - trial_distance_timer >= trial_distance:
                    trial_distance_timer = time.time()
                    trial_distance = random.randint(TRIAL_DISTANCE_RANGE)
                    in_trial = True
                    trial_count += 1
                    trial_led.on()
                    collectData(data, lever_angle, servo_angle, start_time, 0, 0, trial_count)
                else:
                    collectData(data, lever_angle, servo_angle, start_time, 0, 0, 0)
